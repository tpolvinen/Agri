# Talavera 2017: Review of IoT applications in agro-industrial and environmental fields 
...
##### Valvonta-kategorian alikategoriat:
...
Katsauksen tekijät esittivät IoT-arkkitehtuurin yhteenvetona katsauksessa tehdyistä havainnoista. Tekijöiden mukaan arkkitehtuuri kiteyttää suurimman osan katsauksessa käsitellyistä tutkimuksista. Arkkitehtuurissa on neljä tasoa: fyysinen, tietoliikenne, palvelu ja sovellus. 

**Fyysinen taso** sisältää havainnointikerroksen ja hallintakerroksen. **Havainnointikerroksen** tarkoitus on tuottaa arvokasta dataa tarkkailemalla ympäristömuuttujia WSN:in avulla. Tuotettu data lähetetään tietoliikennetasolle paikallisen *gatewayn (?)* kautta. Havainnointikerroksen laitteissa voi olla virtalähteinä joko akut lyhytaikaista käyttöä varten tai aurinkopaneelit laitteden vähäisen virrankulutuksen takia. Hallintakerros toimii *data sink:inä (?)* vastaanottaen informaatiota tietoliikennetasolta tai yksinkertaisimmassa tapauksessa havainnointikerrokselta. Hallintakerroksen vastaanottama informaatio muuttaa toimilaitteiden tilaa, jotka yleensä käyttävät virtalähteenään sähköverkkoa. Havainnointikerroksen ja hallintakerroksen keskellä toimii liikkumiskykyinen robotti, jota voidaan käyttää jos kiinteästi asennetut laitteet eivät jostain syystä ole paras vaihtoehto.

**Tietoliikennetason** tarkoituksena on siirtää informaatiota fyysiseltä tasolta Internetiin, keräten dataa IoT gateway:iltä perustuen joko Ethernetiin tai mobiiliverkkoihin kuten GPRS, 3G, 4G, NB-IoT ja 5G. Tämä taso pitää sisällään field gatewayt ja lähettimet, jotka käyttävät verkkoratkaisuita kuten ZigBee, Bluetooth, NFC, WiFi, LoRA tai Sigfox.
